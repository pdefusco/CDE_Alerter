...
def main(args):
    ...
    GMAIL_APP_PASSWORD = args[4] #"mysecregapppwdhere"
    MAX_JOB_DURATION_SECONDS = int(args[5]) #number of seconds between job start and end that qualifies the job as a lagger
    EMAIL_RECIPIENT_1 = args[6] #me@myco.com
    EMAIL_RECIPIENT_2 = args[7] #mycolleague@myco.com
    
    # Instantiate the Connection to CDE
    cde_connection = cde.CdeConnection(JOBS_API_URL, WORKLOAD_USER, WORKLOAD_PASSWORD, GMAIL_APP_PASSWORD)
    ...
    if len(laggers_df)>0:
        cde_connection.send_email_alert(laggers_df, job_duration_seconds, "pauldefusco@cloudera.com", "bwilcox@cloudera.com")
        ...
        print("An Email was sent to the following recipients: {0}, {1}".format(EMAIL_RECIPIENT_1, EMAIL_RECIPIENT_2))
    else:
